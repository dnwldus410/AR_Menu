...
public class GroundMovement : MonoBehaviour
{
    ...
    private void CreateGround(float intensity)
    {
        if (!(PlayerPrefs.HasKey("Ground")))
        {
            ...
            float groundposition = Mathf.Lerp(-1f, 3f, intensity);
            gameObject.transform.position = new Vector3(0, 0, groundposition);
        ...
        else if(PlayerPrefs.HasKey("Ground"))
        {
            ...
            float groundposition = Mathf.Lerp(-1f, 3f, losingground);
            gameObject.transform.position = new Vector3(0, 0, groundposition);
        }
    }
}
